    ...
    function openInvoiceEditorModal(clientId){
      ...
      window._invoiceDraft = {
        clientId,
        number: (existing && existing.number) || client.invoiceNumber || _invoiceNumberFor(client, (existing && existing.issuedDate) || new Date().toISOString().slice(0, 10)),
...
// ── SERVICES & PRICING BUILDER (SHARED LIST) ──────────────────────────────────
// Operates on the SAME 'siteServices' key the Frontend Manager and the public
// website use (same-origin localStorage), so the owner can add/edit/delete from
// either the dashboard (backend) OR the website editor (frontend) and it stays in
// sync. Each item: {icon,name,color,desc,tags,price}. Tenants get their own
// prefixed copy (their own rate card). Index-based to match the Frontend Manager.
function _svcGet() {
  let items = getData('siteServices');
  if (!Array.isArray(items)) items = [];
  // One-time migration from the earlier 'servicesPricing' key.
  if (!items.length) {
    const legacy = getData('servicesPricing');
    if (Array.isArray(legacy) && legacy.length) {
      items = legacy.map(x => ({ icon:'🏷️', name:x.name||'Service', color:'#1E5BC0', desc:x.desc||'', tags:[], price:x.price||'' }));
      setData('siteServices', items);
      setData('servicesPricing', []); // prevent re-migration
    }
  }
  return items;
}
function renderServicesPricing() {
  ...
  const items = _svcGet();
  if (!items.length) {
    list.innerHTML = '<div class="card" style="text-align:center;padding:40px 24px;color:#64748B"><div style="font-size:34px;margin-bottom:10px">🏷️</div><div style="font-weight:700;color:#0F172A;margin-bottom:4px">No services yet</div><div style="font-size:13.5px">Click <strong>+ Add Service</strong> to build your rate card. These are the same services shown on your public website.</div></div>';
    return;
  }
  list.innerHTML = '<div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(280px,1fr));gap:16px">' + items.map((it, i) => '<div class="card">'
    + '<div style="display:flex;justify-content:space-between;align-items:flex-start;gap:8px">'
    +   '<div style="font-size:16px;font-weight:700;color:#0F172A">'+(it.icon?esc(it.icon)+' ':'')+esc(it.name)+'</div>'
    +   '<div style="display:flex;gap:6px;flex-shrink:0">'
    +     '<button onclick="openServiceForm('+i+')" class="btn btn-outline" style="padding:4px 9px;font-size:11.5px">Edit</button>'
    +     '<button onclick="deleteServiceEntry('+i+')" style="padding:4px 9px;font-size:11.5px;background:none;border:1px solid #FCA5A5;border-radius:6px;color:#DC2626;cursor:pointer;font-weight:600">Delete</button>'
    +   '</div>'
    + '</div>'
    ...
    + ((it.tags&&it.tags.length)?'<div style="margin-top:8px;display:flex;flex-wrap:wrap;gap:4px">'+it.tags.map(t=>'<span style="font-size:10.5px;background:#EEF2FF;color:#4F46E5;padding:2px 7px;border-radius:99px">'+esc(t)+'</span>').join('')+'</div>':'')
    + '</div>').join('') + '</div>';
}
function openServiceForm(i) {
  const items = _svcGet();
  const existing = (i!=null && i>=0) ? items[i] : null;
  ...
  ov.innerHTML = '<div style="background:#fff;border-radius:14px;max-width:520px;width:100%;box-shadow:0 16px 50px rgba(0,0,0,0.25);overflow:hidden;max-height:92vh;display:flex;flex-direction:column">'
    + '<div style="padding:18px 22px;border-bottom:1px solid #e5e7eb;flex:0 0 auto"><h3 style="font-size:17px;font-weight:700;margin:0">'+(existing?'Edit service':'Add service')+'</h3><p style="font-size:12px;color:#64748B;margin:4px 0 0">Shows in your dashboard, the Frontend Manager, and your public website.</p></div>'
    + '<div style="padding:16px 22px;overflow-y:auto;flex:1 1 auto;min-height:0">'
    +   '<div style="display:flex;gap:10px"><div style="flex:0 0 84px"><label class="form-label">Icon</label><input id="svc-icon" class="form-input" style="margin:0 0 12px;text-align:center" placeholder="🏷️" value="'+attrEsc(existing?existing.icon:'')+'"></div>'
    +   '<div style="flex:1"><label class="form-label">Service name</label><input id="svc-name" class="form-input" style="margin:0 0 12px" placeholder="e.g. 1:1 Coaching Package" value="'+attrEsc(existing?existing.name:'')+'"></div></div>'
    ...
    +   '<label class="form-label">What\'s included</label><textarea id="svc-desc" class="form-input" style="margin:0;min-height:120px;resize:vertical;font-family:inherit;line-height:1.55" placeholder="Describe what the client gets…">'+bodyEsc(existing?existing.desc:'')+'</textarea>'
    + '</div>'
    ...
    +   '<button onclick="saveServiceEntry('+(existing?i:'null')+')" class="btn btn-solid" style="padding:8px 16px">'+(existing?'Save changes':'Add service')+'</button>'
    + '</div></div>';
  document.body.appendChild(ov);
  setTimeout(()=>document.getElementById('svc-name')?.focus(),60);
}
function saveServiceEntry(i) {
  ...
  const items = _svcGet();
  if (i!=null && i>=0 && items[i]) {
    items[i].name = name; items[i].icon = icon || items[i].icon || '🏷️'; items[i].price = price; items[i].desc = desc;
  } else {
    items.push({ icon: icon || '🏷️', name, color:'#1E5BC0', desc, tags:[], price });
  }
  setData('siteServices', items);
  document.getElementById('svc-form-overlay')?.remove();
  renderServicesPricing();
  if (typeof showToast==='function') showToast((i!=null&&i>=0)?'Service updated':'Service added','success');
}
function deleteServiceEntry(i) {
  if (!confirm('Delete this service?\n\n⚠️ This is PERMANENT and cannot be undone. It will also be removed from your public website. Are you sure?')) return;
  const items = _svcGet();
  if (i!=null && i>=0) items.splice(i,1);
  setData('siteServices', items);
  renderServicesPricing();
...
// Build an invoice number [4-letter prefix][YYYYMMDD] from a client + date.
// Prefix = first 4 letters of the business name, else first-2 of first name +
// first-2 of last name. Clients sharing a prefix get a stable letter (B, C…).
// Shared by the Reports tab AND the client-portal invoice editor so both match.
function _invoiceNumberFor(client, dateStr) {
  ...
  let prefix = prefixOf(client.businessName || '', client.name || '');
  if (client.id) {
    ...
    const idx = same.findIndex(c => c.id === client.id);
    if (idx > 0) prefix += String.fromCharCode(65 + idx);
  }
  return prefix + ymd;
}

// Auto-fill the Reports → Invoice number field from the selected/typed client + date.
function genInvoiceNumber() {
  ...
  if (el) el.value = _invoiceNumberFor({ businessName: biz, name: name, id: clientId }, dateVal);
}
...
Decide the section count based on the topic — small SOP = 5-8 sections, full curriculum = 15-25 sections. Each section will become 800-1500 words of detailed content. Order sections logically. Return ONLY the JSON object.`;
    ...
    // or have stray commentary around it.
    const _extractJson = (t) => {
      let s = String(t || '').trim();
      if (!s) return null;
      s = s.replace(/```+\s*json/gi, '').replace(/```+/g, '').trim();
      const a = s.indexOf('{'), b = s.lastIndexOf('}');
      if (a === -1 || b === -1 || b < a) return null;
      return s.slice(a, b + 1);
    };
    // For the outline we need PURE JSON — so we use the specialist's expertise
    // but OVERRIDE the "ask clarifying questions first" behavior from the project
    // rules (that's what makes the model reply with questions instead of JSON).
    const outlineSys = (PROJECT_SYSTEM_PROMPTS[_chatProjectKey] || 'You are an expert.')
      + '\n\n## STRUCTURED OUTPUT MODE (this request only)\n'
      + 'Do NOT greet, do NOT ask clarifying questions, do NOT add any commentary or markdown. '
      + 'Use your expertise and best judgment to fill any gaps yourself. '
      + 'Respond with ONLY the requested JSON object — it must start with { and end with } and contain nothing else.';
    let outline = null, rawText = '';
    // Up to 2 attempts — models occasionally return an empty/truncated reply.
    for (let attempt = 0; attempt < 2 && !outline; attempt++) {
      if (attempt > 0) outlineBubble.innerHTML = '⏳ Retrying outline…';
      try {
        const { text } = await askAI({
          messages: [{role:'system', content: outlineSys}, {role:'user', content: outlinePrompt}],
          project: _chatProjectKey
        });
        rawText = String(text || '').trim();
        const cleaned = _extractJson(rawText);
        if (!cleaned) continue;
        const parsed = JSON.parse(cleaned);
        if (parsed && Array.isArray(parsed.sections) && parsed.sections.length) outline = parsed;
      } catch (e) { /* try again, or fall through to the message below */ }
    }
    if (!outline) {
      const reason = rawText
        ? 'the AI didn\'t return a usable outline this time'
        : 'the AI returned an empty response — usually a missing/rate-limited AI key (check Settings → AI Integration) or a busy moment';
      outlineBubble.innerHTML = '<div style="color:#dc2626;padding:12px;background:#fef2f2;border-radius:8px">⚠️ Couldn\'t build the manual outline: ' + reason + '.<br><br>Please try again'
        + (rawText ? ', or rephrase a bit more specifically (e.g. "Build a career-choices manual for ISFP personality types — strengths, ideal fields, and a job-search plan").' : '.')
        + '</div>';
      return;
    }
    ...
    if (!(typeof TENANT !== 'undefined' && TENANT) && typeof API_BASE !== 'undefined') {
      try {
        ...
        const resp = await fetch(API_BASE + '/api/ai', {
          method: 'POST', headers: { 'Content-Type': 'application/json' },
          // The server reads `systemOverride` (not systemPrompt) and streams Groq's
          // native SSE — pass the specialist prompt through so the persona/rules
          // still apply on the fallback.
          body: JSON.stringify({ messages: chatMsgs, systemOverride: sysMsg ? sysMsg.content : undefined })
        });
        if (resp.ok && resp.body) {
          ...
          const handle = (line) => {
            ...
            // Server pipes Groq's raw SSE → delta is at choices[0].delta.content.
            try { const ev = JSON.parse(d); if (ev.error) return; const delta = (ev.choices && ev.choices[0] && ev.choices[0].delta && ev.choices[0].delta.content) || ev.delta || ''; if (delta) { full += delta; if (onChunk) onChunk(delta, full); } } catch {}
          };
  ...
  // If a weaker model parrots the internal kickoff/continue directive back
  // instead of following it, strip it so the user never sees the meta-text.
  function _stripKickoffEcho(s) {
    if (!s) return s;
    s = s.replace(/Start the session now per the FULL instructions[\s\S]*?welcome flow\.?/gi, '');
    s = s.replace(/Continue per your project instructions\.[\s\S]*?next actions\.?/gi, '');
    ...
    try {
      const { text } = await askAI({
        ...
        onChunk: (delta, full) => {
          ...
          streamBubble.innerHTML = mdRender(_stripKickoffEcho(full));
          if (msgs) msgs.scrollTop = msgs.scrollHeight;
        }
      });
      const reply = _stripKickoffEcho(cleanGroqResponse(text)) || 'No response.';
      if (!streamBubble) { typing.remove(); streamBubble = _appendMsg('assistant', ''); }
      streamBubble.classList.add('md-content');
